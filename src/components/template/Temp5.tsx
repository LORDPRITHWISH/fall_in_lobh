...
const Logo = ({ prev = false }: { prev: boolean }) => (
  <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className={`fixed z-800 ${prev ? "top-4 right-4" : "top-6 right-6"}`}>
    <motion.div
      whileHover={{ scale: 1.1, rotate: [0, -10, 10, 0] }}
      className={`flex items-center gap-2 bg-blue-800/20 backdrop-blur-sm ${prev ? "px-2 py-1 rounded-md" : "px-4 py-2 rounded-lg"} border border-blue-800/30`}
    >
      <Heart className={`${prev ? "w-4 h-4" : "w-6 h-6"} text-blue-800`} />
      <span className={`${prev ? "text-sm" : "text-base"} text-blue-800 font-bold`}>LoveBears</span>
    </motion.div>
  </motion.div>
);
...
const BackgroundMagic = () => (
  <div className="fixed inset-0 -z-800 overflow-hidden">
    {/* Animated gradient overlay */}
    <motion.div
      className="absolute inset-0 "
      animate={{
        background: [
          "linear-gradient(45deg, 8000, 8000)",
          "linear-gradient(135deg, 8000, 8000)",
          "linear-gradient(225deg, 8000, 8000)",
          "linear-gradient(315deg, 8000, 8000)",
        ],
      }}
    ...
    <div className="absolute inset-0 grid grid-cols-12 gap-1 opacity-15">
      {Array.from({ length: 144 }).map((_, i) => (
        <motion.div
          ...
          animate={{
            scale: [0.5, 1, 0.5],
            backgroundColor: ["8000", "8000", "8000"],
          }}
          transition={{
    ...
    {Array.from({ length: 10 }).map((_, i) => (
      <motion.div
        key={`bubble-${i}`}
        className="absolute rounded-full border-2 border-blue-800"
        style={{
...
const CelebrationPopup = ({
  ...
  <motion.div
    ...
    animate={{ opacity: 1 }}
    exit={{ opacity: 0 }}
    className="fixed inset-0 flex items-center justify-center z-800 bg-blue-800/90 backdrop-blur-sm"
    onClick={onClose}
  >
    <motion.div
      ...
      className="bg-blue rounded-3xl p-8 max-w-md w-full mx-4 shadow-xl relative overflow-hidden border-2 border-blue-800"
      onClick={(e) => e.stopPropagation()}
    >
      <motion.div
        className="absolute inset-0 opacity-30"
        animate={{
          background: ["linear-gradient(45deg, 8000, 8000)", "linear-gradient(45deg, 8000, 8000)"],
        }}
      ...
      <div className="text-center relative">
        ...
        <motion.div
          ...
          className={`${prev ? "w-32 h-32" : "w-48 h-48"} mx-auto mb-6 rounded-full overflow-hidden border-4 border-blue-800`}
        >
          <Image src={celebrationMediaUrl} alt="Celebration" layout="fill" objectFit="cover" />
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <h2 className={`${prev ? "text-2xl" : "text-3xl"} font-bold text-blue-8000 mb-4`}>Love Activated! 🌈</h2>
          <p className={`text-blue-800 ${prev ? "text-sm" : "text-lg"} mb-6`}>{celebrationMessage}</p>
        </motion.div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className={`${prev ? "px-4 py-2 text-sm" : "px-6 py-2 text-lg"} mt-6 bg-blue-800 text-blue rounded-full font-semibold shadow-lg hover:bg-blue-8000 transition-all`}
...
export default function Temp5({ title, messages, moods, prev = false, noButtonMessages, celebrationMediaUrl, celebrationMessage }: Temp1Props) {
    ...
    return (
      <div className={` flex items-center justify-center p-4 overflow-hidden`}>
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center relative">
          ...
          >
            <Heart className={`${prev ? "w-24 h-24" : "w-32 h-32"} text-blue-800 mb-8 mx-auto`} />
          </motion.div>
          <motion.button
            ...
            } rounded-full bg-blue-800 text-blue font-semibold flex items-center gap-2 mx-auto shadow-lg hover:bg-blue-8000 transition-all`}
          >
            <Heart className={`${prev ? "w-5 h-5" : "w-6 h-6"} fill-blue`} />
            {title}
          </motion.button>
        </motion.div>
      </div>
  ...
  return (
    <div className={`${prev ? "h-[60dvh]" : "min-h-[800dvh]"} overflow-hidden relative `}>
      <BackgroundMagic />
      {/* <Logo prev={prev} /> */}
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={step}
          initial={{ opacity: 0, scale: 0.8, y: 800 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 1.2, y: -800 }}
          transition={{ type: "spring", stiffness: 800, damping: 20 }}
          className="absolute inset-0 flex items-center justify-center p-4"
        >
          <motion.div className="text-center" initial={{ y: 20, rotateX: 90 }} animate={{ y: 0, rotateX: 0 }} transition={{ type: "spring", stiffness: 800 }}>
            ...
            {step === messages.length - 1 && (
              ...
              >
                <motion.button
                  ...
                  } rounded-full bg-blue-800 text-blue font-bold flex items-center gap-2 shadow-lg hover:bg-blue-8000`}
                ...
                <motion.button
                  ...
                  } rounded-full bg-blue/800 text-blue-800 font-bold flex items-center gap-2 border-2 border-blue-800 hover:border-blue-800`}
                >
