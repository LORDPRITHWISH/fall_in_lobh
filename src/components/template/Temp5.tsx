...
export default function Temp4({ title, messages, moods, prev = false, noButtonMessages, celebrationMediaUrl, celebrationMessage }: Temp1Props) {
  ...
  if (!showEmojis) {
    return (
      <div className={`${prev ? "min-h-[60dvh]" : "min-h-[100dvh]"} flex items-center justify-center p-4 overflow-hidden bg-pink-50`}>
        <BackgroundMagic />
        <Logo prev={prev} />
  ...
  return (
    <div className={`${prev ? "h-[60dvh]" : "min-h-[100dvh]"} overflow-hidden relative bg-pink-50`}>
      <BackgroundMagic />
      <Logo prev={prev} />
